...
//
// Shared 4-step job wizard. Drives both the create flow (/jobs/new) and the
// edit flow (/jobs/[id]/edit). Mode-specific behavior is gated on the `mode`
// prop:
//   - 'create' → POST /jobs at step 3 → 4, status stays draft until Activate.
//   - 'edit'   → PATCH /jobs/:id at step 3 → 4. Buttons on step 4 depend on
//                whether the job is currently a draft (Activate visible) or
//                already active (Save changes only).
// Archived jobs cannot reach edit mode — the edit page wrapper redirects.
...
export type WizardFormData = z.infer<typeof schema>

// ── PROPS ─────────────────────────────────────────────────────────────────────
export type JobWizardMode = 'create' | 'edit'

export interface JobWizardProps {
  mode: JobWizardMode
  initialJob?: Job  // required when mode === 'edit'
}

// ── HELPERS ───────────────────────────────────────────────────────────────────
function jobToDefaults(job?: Job): Partial<WizardFormData> {
  if (!job) {
    return {
      ...
      aiVerifyFields: {
        minSalary: false, languages: false, visa: false, nationality: false, immediateJoin: false,
      },
    }
  }
  // Edit mode: hydrate from the existing Job. Fields that aren't persisted on
  // the Job model (employmentType, visa/nationality/immediateJoin prefs,
  // aiVerifyFields) fall back to the create-defaults — they were never stored,
  // so we cannot round-trip them.
  const extracted = (job.extractedCriteria as any) || {}
  return {
    title:              job.title,
    ...
    requiredLanguages:  job.requiredLanguages?.length ? job.requiredLanguages : ['English'],
...
// ── MAIN COMPONENT ────────────────────────────────────────────────────────────
export function JobWizard({ mode, initialJob }: JobWizardProps) {
  ...
  // In edit mode the jobId is fixed; in create mode it gets set after step 3.
  const [createdJobId, setCreatedJobId] = useState<string | null>(initialJob?.id ?? null)
  ...
  const { register, watch, setValue, getValues, formState: { errors } } = useForm<WizardFormData>({
    defaultValues: jobToDefaults(initialJob),
  })

  const vals = watch()
  const heading = isEdit ? 'Edit Job' : 'Post New Job'
  ...
  const computeAiMandatoryFields = (v: WizardFormData): string[] =>
  ...
  // ── STEP 3 → 4: Submit (create or update) ─────────────────────────────────
  const submitStep3 = async () => {
    ...
    try {
      ...
      if (isEdit && createdJobId) {
        const res = await api.patch<any>(`/jobs/${createdJobId}`, payload)
        jobId = res.data.data.id
        questions = res.data.data.screeningQuestions || []
      } else {
        const res = await api.post<any>('/jobs', { ...payload, allowDuplicate })
        jobId = res.data.data.id
        setCreatedJobId(jobId)
        questions = res.data.data.screeningQuestions || []
      }

      // If AI questions weren't ready yet (slow process-jd), poll until they
      // arrive. Same logic in both modes.
      ...
      toast.error(backendMsg
        ? `Couldn't ${isEdit ? 'update' : 'create'} job — ${backendMsg}`
        : `Failed to ${isEdit ? 'update' : 'create'} job. Check that backend is running.`)
      console.error(err)
    } finally {
  ...
  // ── STEP 3: Save as Draft (no advance) ────────────────────────────────────
  // Persists the current wizard state as a draft and navigates back to the
  // jobs list. Does NOT poll for AI baseline questions — the recruiter can
  // resume editing later and the AI will have caught up by then.
  const saveDraftFromStep3 = async () => {
    const v = getValues()
    const jdText = v.jdText || v.generatedJdEn || ''
    ...
    try {
      ...
      if (createdJobId) {
        // Either edit mode, or create mode where the wizard already POSTed
        // earlier and the recruiter came back to step 3 to revise. Either way
        // we PATCH the existing record.
        await api.patch(`/jobs/${createdJobId}`, payload)
      } else {
        await api.post(`/jobs`, { ...payload, allowDuplicate })
      }
      ...
      const backendMsg = err?.response?.data?.error?.message
      toast.error(backendMsg ? `Couldn't save draft — ${backendMsg}` : 'Failed to save draft')
    } finally {
  ...
  const persistScreeningEdits = async () => {
    ...
    const v = getValues()
    await api.patch(`/jobs/${createdJobId}`, {
  ...
  const activateJob = async () => {
    ...
    try {
      ...
      if (isEdit) await persistScreeningEdits()
      ...
      const backendMsg = err?.response?.data?.error?.message
      toast.error(backendMsg ? `Activation failed — ${backendMsg}` : 'Activation failed')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const saveAsDraft = async () => {
    ...
    if (isEdit) {
      ...
      // Create mode: backend already created the draft at step 3 → 4.
      toast.success('Saved as draft')
      router.push('/jobs')
    }
  }

  // Edit mode + already-active: the only valid step-4 action is "Save changes"
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      router.push(`/jobs/${createdJobId}/view`)
    } catch {
  ...
  const cancelEdit = () => {
    if (createdJobId) router.push(`/jobs/${createdJobId}/view`)
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold" style={{color: BRAND.GREEN_DARK}}>{heading}</h1>
          <AiDrivenBadge />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input {...register('title', {
              onBlur: async () => {
                if (isEdit) return  // skip duplicate-check in edit mode (you ARE the job)
      ...
      <div className="flex justify-between mt-5">
        {isEdit ? (
          <button onClick={cancelEdit} className="px-6 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50">
            Cancel
          </button>
        ) : <span />}
        <button onClick={() => {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between mt-5">
          ...
          <div className="flex gap-3">
            {/* Save-as-Draft is exposed only on Step 3 (not steps 1, 2, or 4)
                per product call. Step 4 keeps its own draft button for the
                post-AI-generation save path. */}
            <button onClick={saveDraftFromStep3} disabled={isSubmitting || isGeneratingQuestions}
              className="px-6 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-60">
              {isSubmitting ? 'Saving...' : 'Save as Draft'}
            </button>
            <button onClick={submitStep3} disabled={isGeneratingQuestions || isSubmitting}
              ...
              {isGeneratingQuestions ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>{isEdit ? 'Updating...' : 'Creating job...'}</> : 'Next: Baseline Questions →'}
            </button>
          </div>
        </div>
      </div>
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Action set depends on mode AND current job status:
  //   create              → Save as Draft + Activate
  //   edit on draft       → Save as Draft + Activate
  //   edit on active      → Save changes (PATCH only, no activate)
  const isAlreadyActive = isEdit && initialStatus === JOB_STATUS.ACTIVE
      ...
      <div className="flex justify-between mt-5">
        ...
        <div className="flex gap-3">
          {isAlreadyActive ? (
            ...
            <>
              <button onClick={saveAsDraft} disabled={isSubmitting}
                className="px-6 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-60">
                Save as Draft
              </button>
